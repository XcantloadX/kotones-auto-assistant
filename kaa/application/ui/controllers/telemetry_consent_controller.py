...
统计数据收集 statics）尚未设置（None）且非开发环境，QML 弹窗询问用户；用户选择后
三个开关一并写入 _shared.json，下次启动生效。
"""
...
class TelemetryConsentController(QObject):
    ...
    初始化即读取 shared 配置，将三个开关的当前状态暴露给 QML 弹窗展示；并在
    （非开发环境且任一配置项尚未表态时）将 ``telemetryConsentRequired`` 置为 True。
    用户在弹窗中选择后调用 :meth:`setTelemetryConsent` 持久化三个配置并清除待表态标记。
    """
    ...
    staticsEnabled = Property(bool, _get_statics, notify=staticsEnabledChanged)
    ...
    def setTelemetryConsent(self, sentry: bool, screenshot: bool, statics: bool) -> None:
        ...
        """
        telemetry.set_consent(bool(sentry), bool(screenshot), bool(statics))
        self._sentry = bool(sentry)
        self._screenshot = bool(screenshot)
        self._statics = bool(statics)
        ...
        logger.info(
            'Telemetry consent settled: sentry=%s screenshot=%s statics=%s',
            sentry, screenshot, statics,
        )